...
export default function SignupPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    e.preventDefault()
    if (!tradeCategory) { toast.error('Please select your trade category'); return }
    setLoading(true)
    ...
    try {
      ...
      const res = await fetch('/api/workers', {
        ...
        body: JSON.stringify({ display_name: displayName, slug, trade_category: tradeCategory }),
      })
              ...
              <>
                ...
                <div>
                  ...
                  <input type="text" value={displayName} onChange={e => setDisplayName(e.target.value)}
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-ink-600 focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all"
                    placeholder="Alex Johnson" required minLength={2} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-ink-300 mb-2">Your trade</label>
                  <div className="grid grid-cols-2 gap-2">
                    {TRADE_CATEGORIES.map(cat => (
                      <button key={cat} type="button" onClick={() => setTradeCategory(cat)}
                        className={`px-3 py-2.5 rounded-xl text-sm font-medium text-left transition-all border ${
                          tradeCategory === cat ? 'bg-brand-600/20 border-brand-500 text-brand-300' : 'bg-white/5 border-white/10 text-ink-400 hover:border-white/20 hover:text-white'
                        }`}>
                ...
                <div className="flex gap-3">
                  ...
                  <button type="submit" disabled={loading || !displayName || !tradeCategory}
